...
interface ParticleTextEffectProps {
  words?: string[]
  /** Renders just the canvas sized to its parent — no wrapper div, no black bg */
  embedded?: boolean
}
...
export function ParticleTextEffect({ words = DEFAULT_WORDS, embedded = false }: ParticleTextEffectProps) {
  ...
  const nextWord = (word: string, canvas: HTMLCanvasElement) => {
    ...
    // Scale font to canvas width so it fills nicely at any size
    const fontSize = Math.max(40, Math.floor(canvas.width * 0.13))
